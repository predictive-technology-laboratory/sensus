...
namespace Scripter
{
    public partial class Scripter : Form
    {
        ...
        private void Scripter_Load(object sender, EventArgs e)
        {
            ...
            textOutputOnly.Prompts.Add(new Prompt(PromptOutputType.Text, "Hi", PromptInputType.None));
            textOutputOnly.Save(@"..\..\Scripts\ExampleTextOutput.json");

            Script textPrompt = new Script("Text Prompt");
            textPrompt.Prompts.Add(new Prompt(PromptOutputType.Text, "How are you?", PromptInputType.Text));
            textPrompt.Save(@"..\..\Scripts\ExampleTextPrompt.json");

            Script longVoicePrompt = new Script("Long Voice Prompt");
            longVoicePrompt.Prompts.Add(new Prompt(PromptOutputType.Voice, "This is a long voice prompt. It should take at least a little while to complete. If it's not long enough, use the scripter to make it even longer.", PromptInputType.Voice, true, 3));
            ...
            voiceOutputOnlyDelayed.Prompts.Add(new Prompt(PromptOutputType.Voice, "Hi", PromptInputType.None));
            voiceOutputOnlyDelayed.Save(@"..\..\Scripts\ExampleVoiceOutputDelayed.json");

            Script voicePrompt = new Script("Voice Prompt");
            voicePrompt.Prompts.Add(new Prompt(PromptOutputType.Voice, "How are you?", PromptInputType.Voice));
