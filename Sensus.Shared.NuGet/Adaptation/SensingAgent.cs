...
namespace Sensus.Adaptation
{
    ...
    public abstract partial class SensingAgent : INotifyPropertyChanged
    {
        ...
        /// <see cref="SensingAgentState.OpportunisticObservation"/> or during 
        /// <see cref="SensingAgentState.ActiveObservation"/>.
        /// </summary>
        ...
        private async Task<bool> TransitionToNewStateAsync(SensingAgentState newState, CancellationToken cancellationToken)
        {
            ...
            if (stateChanged)
            {
                ...
                {
                    SensusServiceHelper.Logger.Log("Exception while notifying concrete class of change to state " + State + ":  " + ex.Message, LoggingLevel.Normal, GetType());
                    throw ex;
                }
                finally
                {
