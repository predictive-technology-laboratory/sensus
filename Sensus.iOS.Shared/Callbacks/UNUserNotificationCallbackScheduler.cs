...
namespace Sensus.iOS.Callbacks
{
    public class UNUserNotificationCallbackScheduler : iOSCallbackScheduler
    {
        ...
        protected override async Task RequestLocalInvocationAsync(ScheduledCallback callback)
        {
            ...
            {
                await notifier.IssueNotificationAsync(callback.Protocol?.Name ?? "Alert", callback.UserNotificationMessage, callback.Id, true, callback.Protocol, null, callback.DisplayPage, callback.NextExecution.Value, callbackInfo, requestCreated);
            }
        }
