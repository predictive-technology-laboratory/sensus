...
using Sensus.Shared.Android.Probes.Context;

namespace Sensus.Shared.Android
{
    public class AndroidSensusServiceHelper<MainActivityT> : SensusServiceHelper where MainActivityT : FormsApplicationActivity
    {
        private AndroidSensusService<MainActivityT> _service;
        private ConnectivityManager _connectivityManager;
        private string _deviceId;
        private MainActivityT _focusedMainActivity;
        private readonly object _focusedMainActivityLocker = new object();
        private AndroidTextToSpeech<MainActivityT> _textToSpeech;
        private PowerManager.WakeLock _wakeLock;
        private int _wakeLockAcquisitionCount;
        private List<Action<MainActivityT>> _actionsToRunUsingMainActivity;
        private bool _userDeniedBluetoothEnable;

        public AndroidSensusService<MainActivityT> Service
        {
            get { return _service; }
            set
            {
                ...
                else
                {
                    ...
                    _textToSpeech = new AndroidTextToSpeech<MainActivityT>(_service);
...
#pragma warning restore 618
        ...
        {
            _actionsToRunUsingMainActivity = new List<Action<MainActivityT>>();
        ...
        public void RunActionUsingMainActivityAsync(Action<MainActivityT> action, bool startMainActivityIfNotFocused, bool holdActionIfNoActivity)
        {
                    ...
                    lock (_focusedMainActivityLocker)
                    {
                        ...
                        if (_focusedMainActivity == null)
                        {
                            if (startMainActivityIfNotFocused)
                            {
                                ...
                                Intent intent = new Intent(_service, typeof(MainActivityT));
        ...
        public void SetFocusedMainActivity(MainActivityT focusedMainActivity)
        {
            lock (_focusedMainActivityLocker)
            {
        ...
        private void RunActionsUsingMainActivity()
        {
            lock (_focusedMainActivityLocker)
            {
                lock (_actionsToRunUsingMainActivity)
                {
                    ...
                    foreach (Action<MainActivityT> action in _actionsToRunUsingMainActivity)
